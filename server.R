...
shinyServer(function(input, output, session) {
  ...
  process <- reactiveValues(active = FALSE)
  ...
  # Initialize shinyFiles directories
  ...
  antropometrie_path <- reactive({ parseDirPath(volumes, input$antropometrie)})
  wingate_path <- reactive({ parseDirPath(volumes, input$wingate_path) })
  spirometrie_path <- reactive({ parseDirPath(volumes, input$spirometrie_path) })
  srovnani_path <- reactive({ parseDirPath(volumes, input$srovnani_path) })
  srovnani2_path <- reactive({ parseDirPath(volumes, input$srovnani2_path) })
  antropometrie_files <- reactiveVal(c(NULL))
  
  # Define file_lists reactive expression
  file_lists <- reactive({
    # Add dependency on the reactive variable
    refreshFiles
    input$refresh_files
    
    antropometrie_path <- parseDirPath(volumes, input$antropometrie)
    wingate_path <- if (!is.null(input$wingate) && input$wingate) parseDirPath(volumes, input$wingate_path) else NULL
    spirometrie_path <- if (!is.null(input$spirometrie) && input$spirometrie) parseDirPath(volumes, input$spirometrie_path) else NULL
    srovnani_path <- if (!is.null(input$srovnani) && input$srovnani) parseDirPath(volumes, input$srovnani_path) else NULL
    srovnani2_path <- if (!is.null(input$srovnani2) && input$srovnani2) parseDirPath(volumes, input$srovnani2_path) else NULL
    
    
    if (length(antropometrie_path) > 0) {
    ...
    # List files in each path and extract just the filenames without the .txt extension
    antropometrie_files <- if (!is.null(antropometrie_path)) {
      files <- list.files(path = antropometrie_path, pattern = '\\.xls[xm]$', full.names = TRUE)
      
      if (length(files) > 0) {
        # Read the first Excel file and extract IDs from the "Data_Sheet" sheet
        file <- files[1]
        data <- readxl::read_excel(file, sheet = "Data_Sheet")
        
        check_dates <- function(data) {
          values_birth <- base::as.Date(data$Birth, format = "%d/%m/%Y")
          values_date_measurement <- base::as.Date(data$Date_measurement, format = "%d/%m/%Y")
        ...
        # Check dates and get invalid IDs
        wrong_format <- check_dates(data)
        date_check(wrong_format)
        
        
        if ("ID" %in% colnames(data)) {
          id_values <- na.omit(data$ID)  # Extract non-missing IDs
          id_values
        } else {
    ...
    # Check for the SJ column and its length
    squat_jumps_count <- if (!is.null(antropometrie_path)) {
      files <- list.files(path = antropometrie_path, pattern = '\\.xls[xm]$', full.names = TRUE)
      if (length(files) > 0) {
        # Read the first Excel file and extract IDs from the "Data_Sheet" sheet
        file <- files[1]
        ...
        if ("SJ" %in% colnames(data)) {
          ...
          length(na.omit(data$SJ))  # Count non-missing SJ entries
        } else {
          "SJs not present"
        }
      } else {
    ...
    wingate_files <- if (!is.null(wingate_path)) {
      files <- list.files(path = wingate_path, pattern = '*.txt', full.names = TRUE)
      file_names <- basename(files)
      sub("\\.txt$", "", file_names)
    } else NULL
    
    srovnani_files <- if (!is.null(srovnani_path)) {
      files <- list.files(path = srovnani_path, pattern = '*.txt', full.names = TRUE)
      file_names <- basename(files)
      sub("\\.txt$", "", file_names)
    } else NULL
    
    srovnani2_files <- if (!is.null(srovnani2_path)) {
      files <- list.files(path = srovnani2_path, pattern = '*.txt', full.names = TRUE)
      file_names <- basename(files)
      sub("\\.txt$", "", file_names)
    } else NULL
    
    spirometrie_files <- if (!is.null(spirometrie_path)) {
      files <- list.files(path = spirometrie_path, pattern = '*.xls', full.names = TRUE)
      file_names <- basename(files)
      sub("\\.(xls|xlsx)$", "", file_names)
    } else NULL
  ...
  printFileList <- function(file_list) {
    output$file_list_output <- renderPrint({
      # Initialize an empty string to accumulate output
      output_text <- ""
      
      # Check if each checkbox is selected and add counts accordingly
      antropometrie_path <- parseDirPath(volumes, input$antropometrie)
      if (!is.null(antropometrie_path)) {
        output_text <- paste0(output_text, "Anthropometry Files: ", length(file_list$Antropometrie), "\n")
      }
      
      if (input$wingate) {
        output_text <- paste0(output_text, "Wingate Files: ", length(file_list$Wingate), "\n")
      }
      
      if (input$spirometrie) {
        output_text <- paste0(output_text, "Spirometrie Files: ", length(file_list$Spirometrie), "\n")
      }
      
      if (!is.null(file_list$SquatJumps)) {
        output_text <- paste0(output_text, "Squat Jumps: ", file_list$SquatJumps, "\n")
      }
      
      if (input$srovnani) {
        output_text <- paste0(output_text, "Comparison Files: ", length(file_list$Srovnani), "\n")
      }
      
      if (input$srovnani2) {
        output_text <- paste0(output_text, "Comparison 2 Files: ", length(file_list$Srovnani2), "\n")
      }
  ...
  an_path_reactive <- reactive({
    parseDirPath(volumes, input$antropometrie)
  })
  
   # Render paths and file lists in the UI
  output$antropometrie_path <- renderPrint({
    req(length(an_path_reactive()) > 0)
    an_path_reactive()
  })
  
  win_path_reactive <- reactive({
    parseDirPath(volumes, input$wingate_path)
  })
  
  output$wingate_path_display <- renderPrint({
    if (input$wingate) {
      req(length(win_path_reactive()) > 0)
      win_path_reactive()
    }
  })
  
  srovnani_path_reactive <- reactive({
    parseDirPath(volumes, input$srovnani_path)
  })
  
  output$srovnani_path_display <- renderPrint({
    if (input$srovnani) {
      req(length(srovnani_path_reactive()) > 0)
      srovnani_path_reactive()
    }
  })
  
  srovnani2_path_reactive <- reactive({
    parseDirPath(volumes, input$srovnani2_path)
  })
  
  
  output$srovnani2_path_display <- renderPrint({
    if (input$srovnani2) {
      req(length(srovnani2_path_reactive()) > 0)
      srovnani2_path_reactive()
    }
  })
  
  spiro_path_reactive <- reactive({
    parseDirPath(volumes, input$spirometrie_path)
  })
  
  output$spirometrie_path_display <- renderPrint({
    if (input$spirometrie) {
      req(length(spiro_path_reactive()) > 0)
      spiro_path_reactive()
    }
  })
  ...
  output$spatne_spiro <- renderPrint({
  ...
  observeEvent(input$submit, {
    ...
    sport <- input$sport 
    antropometrie_path <- parseDirPath(volumes, input$antropometrie)
    wingate_path <- parseDirPath(volumes, input$wingate_path)
    spirometrie_path <- parseDirPath(volumes, input$spirometrie_path)
    comparison.path <- parseDirPath(volumes, input$srovnani_path)
    comparison_2.path <- parseDirPath(volumes, input$srovnani2_path)
    ...
    spatne.wingate <- trimws(spatne.wingate())
    spatne.compar <- trimws(spatne.compar())
    ...
    if (length(chybne.pocty) > 0) {
      # Add the values from chybne.pocty() to spatne.compar and spatne.compar_2
      spatne.compar <- c(spatne.compar, chybne.pocty)
      spatne.compar <- unique(spatne.compar)
      spatne.compar_2 <- c(spatne.compar_2, chybne.pocty)
    ...
    dotaz_spiro <- ifelse(dotaz_spiro == T, "YES", "NO")
    srovnani <- ifelse(srovnani == T, "YES", "NO")
    srovnani2 <- ifelse(srovnani2 == T, "YES", "NO")
    tri_graf <- ifelse(tri_graf == T, "YES", "NO") 
    
    file.path <- wingate_path #Wingate folder
    file.path.an <-  antropometrie_path  #Anthropometry folder
    report.path <- paste(wd, "/reporty", sep = "") #Reports folder
    database.path <- paste(wd, "/databaze", sep = "") #Database folder
    spiro.path <- spirometrie_path  #Spiroergometry folder
    
 
    # Create file lists
    file.list <- basename(list.files(path = file.path, pattern = '*.txt', full.names = TRUE))
    file.list.an <- basename(list.files(path = file.path.an, pattern = '\\.xls[xm]$', ignore.case = TRUE))
    file.list.dat <- basename(list.files(path = database.path, pattern = c('*.xls')))
    ...
    if (srovnani2 == "YES") {
      file.list.compar_2.bez <- basename(list.files(path = comparison_2.path, pattern = '*.txt', full.names = TRUE)) 
      file.list.compar_2.bez <- sub("\\.txt$", "", file.list.compar_2.bez)
    if (length(spatne.compar_2) > 0) {
      file.list.compar_2.bez <- file.list.compar_2.bez[!file.list.compar_2.bez %in% spatne.compar_2]
      if (length(file.list.compar_2.bez) < 1) {
        file.list.compar_2.bez <- NA
      }
    }
    } else {
    ...
    process_and_generate_pdfs <- function(wingate_path, antropometrie_path, spirometrie_path, N, incProgress, units_switch) {  
    ...
    process_and_generate_spiro <- function(antropometrie_path, spirometrie_path, N, incProgress, units_switch) {
      ...
      for(i in 1:length(file.list.spiro)) {
        ...
        gtsave(
          table3,
          paste0(program_slozka, "/t3.html"), # Save as HTML
          inline_css = TRUE # Ensure styles are applied
        )
        
        webshot(
          url = paste0(program_slozka, "/t3.html"), # Path to the saved HTML
          file = paste0(program_slozka, "/t3.png"), # Save as PNG
          vwidth = 1600,  # Set viewport width
          vheight = 650,
          zoom = 2,
          cliprect = c(0, 0, 1600, 670)# Set viewport height
        )
    ...
    process_and_generate_spiro(antropometrie_path, spirometrie_path, N, incProgress, units_switch)
      ...
      withProgress(message = 'Calculation in progress', {
        N <- length(file.list)
        process_and_generate_pdfs(wingate_path, antropometrie_path,spirometrie_path, N, incProgress, units_switch)
      })
    }
