...
shinyServer(function(input, output, session) {
  ...
  # program_slozka <- "C:/Users/Dominik Kolinger/Documents/R/shiny/Wingate 22_7_25_update"
  ...
  observeEvent(input$main_folder, {
    ...
    if (length(selected_path) > 0 && dir.exists(selected_path)) {
      ...
      # Získáme všechny složky v selected_path (ne rekurzivně)
      all_dirs <- list.dirs(selected_path, recursive = FALSE, full.names = TRUE)
      
      # Filtrování složek podle názvu (začínající na somato, wingate, spiro)
      somato_dirs <- grep("^somato", basename(all_dirs), value = TRUE)
      wingate_dirs <- grep("^wingate", basename(all_dirs), value = TRUE)
      spiro_dirs <- grep("^spiro", basename(all_dirs), value = TRUE)
      
      somato_path <- if (length(somato_dirs) > 0) file.path(selected_path, somato_dirs[1]) else NULL
      wingate_path <- if (length(wingate_dirs) > 0) file.path(selected_path, wingate_dirs[1]) else NULL
      spiro_path <- if (length(spiro_dirs) > 0) file.path(selected_path, spiro_dirs[1]) else NULL
      
      # Podsložky srovnani a srovnani2 uvnitř wingate složky
      srovnani_path <- NULL
      srovnani2_path <- NULL
      
      if (!is.null(wingate_path) && dir.exists(wingate_path)) {
        wingate_subdirs <- list.dirs(wingate_path, recursive = FALSE, full.names = TRUE)
        srovnani_path <- wingate_subdirs[basename(wingate_subdirs) == "srovnani"]
        srovnani2_path <- wingate_subdirs[basename(wingate_subdirs) == "srovnani2"]
      }
      
      # Ulož cesty do autoPaths
      autoPaths$antropometrie <- somato_path
      autoPaths$wingate <- wingate_path
      autoPaths$spiro <- spiro_path
      autoPaths$srovnani <- if (length(srovnani_path) > 0) srovnani_path else NULL
      autoPaths$srovnani2 <- if (length(srovnani2_path) > 0) srovnani2_path else NULL
      ...
      for(i in 1:length(file.list)) {
        ...
        if (dotaz_spiro == "YES") {
          ...
          if (length(file.list.spiro) != 0) {
            ...
            spiro <- spiro[-c(1,2), ]
            if ("Zátěž" %in% spiro$Fáze) {
              spiro <- subset(spiro, Fáze == "Zátěž")
            } else if ("Exercise" %in% spiro$Fáze) {
              spiro <- subset(spiro, Fáze == "Exercise")
            }
        ...
        if (any(!is.na(file.list.compar_2.bez))) {
          if (srovnani == "YES" & id %in% file.list.compar_2.bez) {
            ...
            df2$`Work (kJ)` <- round(mean(compare.wingate_2$AvP_dopocet*30)/1000,0)
            df2$`Work (J/kg)` <- round(mean(compare.wingate_2$AvP_dopocet*30)/antropo$Weight[length(antropo$Weight)-2],0)
        ...
        if (any(!is.na(file.list.compar.bez))) {
          if (srovnani == "YES" & id %in% file.list.compar.bez) {
            ...
            df2$`Work (kJ)` <- append(na.omit(df2$`Work (kJ)`), round(mean(compare.wingate$AvP_dopocet*30)/1000,1))
            df2$`Work (J/kg)` <- append(na.omit(df2$`Work (J/kg)`), round(mean(compare.wingate$AvP_dopocet*30)/antropo$Weight[length(antropo$Weight)-1],0))
        ...
        df2$`Work (kJ)` <- append(na.omit(df2$`Work (kJ)`), round(totalw/1000,1))
        df2$`Work (J/kg)` <- append(na.omit(df2$`Work (J/kg)`), round(totalw/vaha,0))
        ...
        table2 <- df2 %>% gt() %>% tab_header(title = "Historie měření") %>% 
          ...
          cols_width(
            c("Fat (%)", "ATH", "Weight", "IU (%)") ~ pct(6),
            c("Work (J/kg)","Avg_power (W)", "La_max (mmol/l)") ~ pct(10.5),
            c("Pmax_5s (W)", "Pmax (W)", "Work (kJ)") ~ pct(8),
            c("Date_meas.") ~ pct(7.2),
        ...
        if (dotaz_spiro == "YES") {
          ...
          table3 %>%
            gt::gtsave(paste0(program_slozka, "/t3.png"), vwidth = 1700, vheight = 1000) 
        }
        ...
        radarchart2 <- function(df, axistype=0, seg=4, pty=16, pcol=1:8, plty=1:6, plwd=1,
          ...
          # 2. Vykreslíme plochu (všimněte si, že jsme odstranili main=title)
          plot(c(-1.5, 1.5), c(-1.5, 1.5), type="n", frame.plot=FALSE, axes=FALSE, 
               xlab="", ylab="", main="", asp=1, ...) 
          
          # 3. Ručně vykreslíme HLAVNÍ NADPIS výše (line = 8)
          mtext(title, side = 3, line = 8, cex = 2, font = 2)
          
          # 4. Definice a vykreslení PODTITULU (line = 2 ho dá mezi nadpis a graf)
          subtitle_text <- "Tento síťový graf poskytuje ucelený přehled o kondičním profilu sportovce prostřednictvím srovnání klíčových výkonnostních ukazatelů. Vzdálenost bodů od středu značí úroveň dosaženého výkonu v procentech náležité hodnoty pro danou kategorii, což umožňuje identifikovat, ve kterých oblastech hráč vykazuje silné stránky a kde je naopak prostor pro zlepšení."
          
          # Šířku width = 70 zvolíme, aby byl text užší a písmo mohlo být větší
          subtitle_wrapped <- paste(strwrap(subtitle_text, width = 100), collapse = "\n")
          
          # Vykreslíme podtitul větším písmem (cex = 1.2)
          mtext(subtitle_wrapped, side = 3, line = 1, cex = 1.2, font = 1)# define x-y coordinates without any plot
